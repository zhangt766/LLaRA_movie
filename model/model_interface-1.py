...
class MInterface(pl.LightningModule):
    ...
    def score_demo(self, input):
        ...
        try:
            ...
            # 获取分数最高的前5个键
            top_5_idx = [int(re.findall(r'\d+', item[0])[0]) - 1 for item in sorted_items[:1]]
            return top_5_idx
        except:
            print("bad format, cant decode")
        return random.sample(range(10), 1)
    ...
    def format_fn(self, input):
        if not self.unsloth:
            ...
            similar_historys = input["most_similar_seq_name"][:1]
            similar_choices = input["most_similar_seq_next_name"][:1]
