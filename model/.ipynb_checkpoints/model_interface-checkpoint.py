...
class MInterface(pl.LightningModule):
    ...
    def score_demo(self, input):
        ...
        try:
            ...
            # 获取分数最高的前5个键
            top_5_idx = [int(re.findall(r'\d+', item[0])[0]) - 1 for item in sorted_items[:10]]
            return top_5_idx
        except:
    ...
    def format_fn(self, input):
        if not self.unsloth:
            ...
            similar_historys = input["most_similar_seq_name"][:10]
            similar_choices = input["most_similar_seq_next_name"][:10]
